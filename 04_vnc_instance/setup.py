...
def run_remote_instance(session, configs, cleanup_info):
    ...
    try:
        ...
        stdin, stdout, stderr = ssh.exec_command(install_dependency_cmd, get_pty=True)
        for line in iter(stdout.readline, ""):
            print(line, end="")
        print('install.sh finished.')
        exit_status = stdout.channel.recv_exit_status()

        # setup ec2 instance and start running
        setup_cmd = 'nohup bash setup.sh >/dev/null 2>&1 &'
...
def main(argv):
    ...
    if not cfg.success():
        ...
        try:
            ...
            auth_vnc_response = security_group.authorize_ingress(IpProtocol="tcp",
                                    CidrIp="0.0.0.0/0",FromPort=5901,ToPort=5901)
            ...
            print('connect vnc on: '+instance.public_dns_name+':1')
